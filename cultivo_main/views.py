...
def home(request):
    return render(request,'cultivo_main/agri.html')
def details(request):
    return render(request,'cultivo_main/details.html')
def gallery(request):
    return render(request,'cultivo_main/gallery.html')
def about(request):
    return render(request,'cultivo_main/about.html')
def user(request):
    return render(request,'cultivo_main/user.html')
    
def api_for_weather(place):
    
    result=requests.get('http://api.openweathermap.org/data/2.5/weather?q='+place+'&appid=348faf3ed79a29bad44e712956cce9ec')
    data=result.json()
    return data

def api_for_weather_2(lat,longi):
    
    result=requests.get('http://api.openweathermap.org/data/2.5/weather?lat='+str(lat)+'&lon='+str(longi)+'&appid=348faf3ed79a29bad44e712956cce9ec')
...
def get_soil_info(place, coord):
    def api_for_geocon(place):
        ...
        result = requests.get('https://api.weatherbit.io/v2.0/forecast/agweather?lat=' + str(coord[0]) + '&lon=' + str(
            coord[1]) + '&key=7347cb4e61274bc2bb0c41ad21a0d69f')
        data = result.json()
        return data
...
def finding_subs(p,crop1):
    ...
    for i in p.values():
        ...
        if first_datset.count()!=0 and sec_datset.count()!=0 and third_datset.count()!=0:
            ...
            import pandas as pd
            df=pd.read_csv(r"C:\Users\SANJAY\OneDrive\Desktop\crop\project\Crop_PredictionNew\Agriculture_ml\Core_ML\Crop_Prediction\Dataset3.csv")
            import matplotlib.pyplot as plt
            plt.plot(df.Imports,df.Export)
            plt.show()
            plt.bar(df.Export,df.Domestic)
            plt.show()
            plt.scatter(df.Imports,df.Production)
            plt.show()
...
def resetPassword(request):
    if request.method == 'POST':
        ...
        return render(request, 'cultivo_main/resetPassword.html')
...
def crop(request):
    if request.method=="POST":
        ...
        df=pd.read_csv(r"E:\Agriculture_ml\cultivo_main\static\Crop_recommendation.csv")
        X=df[["Nitrogen","Phosphorous","Potassium","temperature","humidity","ph","rainfall"]]
        y=df[["label"]]
        d=DecisionTreeClassifier()
        d.fit(X,y)
        a=d.predict([[n,p,k,temp,humid,pH,rainfall]])
        return render(request,'cultivo_main/crop.html',{"n":n,"p":p,"k":k,"temp":temp,"humid":humid,"pH":pH,"rainfall":rainfall,"a":a})

def crop_input(request):
    return render(request,"cultivo_main/crop_input.html")